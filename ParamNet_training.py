...
def get_amp_device_type(device: str) -> str:
    return "cuda" if is_cuda_device(device) else "cpu"


def scaler_is_enabled(scaler) -> bool:
    return (scaler is not None) and hasattr(scaler, "is_enabled") and scaler.is_enabled()
...
class BrownianDataset(Dataset):
    def __init__(
        ...
        # Normalize each full trajectory once, then slice windows.
    ...
    def __getitem__(self, idx):
        ...
        targets = np.array(
            [self.k0_log[sample_idx], self.gamma_log[sample_idx]],
            dtype=np.float32,
        )
...
class TemporalMSResStack(nn.Module):
    def __init__(self, in_ch=2, hidden_ch=128, depth=3, dropout=0.1):
        ...
        self.blocks = nn.ModuleList(
            [TemporalMSResBlock(hidden_ch, dilation_base=2 ** i, dropout=dropout) for i in range(depth)]
        )
...
class CrossDomainAttention(nn.Module):
    def __init__(self, time_dim, freq_dim, hidden=128, dropout=0.1):
        ...
        self.t_gate = nn.Sequential(nn.Linear(hidden, time_dim), nn.Sigmoid())
        self.f_gate = nn.Sequential(nn.Linear(hidden, freq_dim), nn.Sigmoid())
...
def nll_gauss_2d(logk_pred, logg_pred, s2k, s2g, tgt_logk, tgt_logg):
    logk_pred = logk_pred.float()
    logg_pred = logg_pred.float()
    s2k = s2k.float()
    s2g = s2g.float()
    tgt_logk = tgt_logk.float()
    tgt_logg = tgt_logg.float()
...
def train_epoch(
    ...
    amp_device = get_amp_device_type(device)
    amp_enabled = scaler_is_enabled(scaler)
...
def validate(
    ...
    return (
        ...
        np.concatenate(k_errs).mean(),
        np.concatenate(g_errs).mean(),
        corr_mean,
    )
...
def train_model(config: TrainingConfig):
    ...
    use_cuda = is_cuda_device(config.device)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        tr = train_epoch(
            ...
            w_ar=config.ar_loss_weight,
            w_acf=config.acf_loss_weight,
            acf_lag_pool=config.acf_lag_pool,
            scaler=scaler,
            ema=ema,
        ...
        print(
            ...
            f"phys_w:{phys_w:.3f} | w_ar:{config.ar_loss_weight:.3f} | "
            f"w_acf:{config.acf_loss_weight:.3f} | "
            f"k:{k_err * 100:.2f}% γ:{g_err * 100:.2f}% | "
            f"|corr(logγ,logP)|:{gp_corr:.3f}"
        )
            ...
            torch.save(
                {
                    ...
                    "physics_weight_epoch": phys_w,
                    "ar_loss_weight": config.ar_loss_weight,
                    "acf_loss_weight": config.acf_loss_weight,
                    "acf_lag_pool": config.acf_lag_pool,
                    "config": vars(config),
    ...
    checkpoint = torch.load(f"{config.save_dir}/best_model.pth", map_location=config.device, weights_only=False)
...
if __name__ == "__main__":
    ...
    val_loader = DataLoader(
        ...
        batch_size=config.batch_size,
        shuffle=False,
        num_workers=min(4, max(0, config.num_workers)),
        pin_memory=use_cuda,
    )
